...
- **Gotchas baked into the scripts**: camstream ignores `SIGHUP` (survives the launching ssh/boot shell) and the launcher must `pkill -f /opt/camstream` (NOT `-f camstream`, which matches the `camstream.sh` script's own cmdline and self-kills); it's `setsid`-detached. Single viewer at a time.
...
- **H.264 status — BLOCKED on SPS/PPS (catch-22), MJPEG is the shipping path.** Detailed RE findings (CedarC v1.2.0):
  ...
  - **Conclusion / recommended path**: cedar-native H.264 needs either Dreame's cloud creds (to run a real session and trace it) or much deeper RE of the `RealyVideoMonitor` message handler. **For a local H.264 feed, transcode the working MJPEG on a LAN host** (ffmpeg/gstreamer `x264enc` → RTSP) — `MJPEG → ffmpeg → RTSP H.264`. The hard part (getting frames off the robot, cloud-free) is already solved by the MJPEG stream.
