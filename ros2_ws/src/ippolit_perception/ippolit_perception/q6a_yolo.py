...
/ qai_appbuilder. The model is exported with a fused detection head, so its outputs are already
decoded:
...
qai_appbuilder does float I/O: we feed normalised RGB in [0,1] (it quantises to the graph's
uint16 input) and it returns dequantised float outputs. Import is lazy so the streamer still
runs if the NPU stack or the model binary is missing (detector just stays disabled).
"""
import os
...
# Prefer the w8a8 build (~45% faster HTP inference, ~22->~12ms; identical on confident
# detections, softer on marginal <0.5-conf ones). Fall back to the w8a16 build if the w8a8
# binary isn't deployed.
...
class YoloDetector:
    def __init__(self, model=MODEL_BIN, labels=LABELS_TXT, conf=0.30, iou=0.45):
        from qai_appbuilder import (
            DataType, LogLevel, ProfilingLevel, QNNConfig, QNNContext, Runtime)
        QNNConfig.Config(Runtime.HTP, LogLevel.WARN, ProfilingLevel.OFF)  # bundled 2.42 v68
        # w8a8's input is quantized uint8 (scale 1/255), so feeding the raw uint8 letterbox as
        # NATIVE input is bit-identical to the float[0,1] path (verified: same boxes/conf) but
        # replaces the ~5-8ms copyFromFloatToNative quantize with a ~0.3ms memcpy. Only valid for
        # the 8-bit-input w8a8 model; the w8a16 fallback keeps float I/O (its native input is
        # 16-bit, a different encoding).
        self.native = str(model).endswith('_w8a8.bin')
        if self.native:
            self.ctx = QNNContext('yolov8_det', model, input_data_type=DataType.NATIVE,
                                  output_data_type=DataType.FLOAT)
        else:
            self.ctx = QNNContext('yolov8_det', model)   # YOLOv8 (v11 not on v68); float I/O
        self.conf = conf
        self.iou = iou
        self.labels = ([line.strip() for line in open(labels) if line.strip()]
    ...
    def _letterbox(self, rgb):
        """
        Resize keeping aspect ratio, pad bottom-right to IN x IN.

        This is the qai-hub YOLO convention -- the model scores much higher this way than with
        centered padding. Returns (img, scale, 0, 0).
        """
        h, w = rgb.shape[:2]
        s = min(IN / w, IN / h)
    ...
    def _map_outputs(self, out):
        ...
        if self.onames and len(self.onames) == len(arrs):                 # map by name if given
            ...
            cl = next((v for k, v in by.items()
                       if 'class' in k or 'idx' in k or 'label' in k), None)
            if b is not None and sc is not None and cl is not None:
                return b.reshape(-1, 4), sc, cl
        # fallback by shape/values: the 4N array is boxes; of the two N arrays, the
        # integer-valued one (all in [0,80)) is class_idx and the other is scores
        ...
        cl = next((a for a in flats
                   if a.size and a.max() < 80 and np.allclose(a, np.round(a), atol=1e-2)), None)
    ...
    def infer(self, rgb):
        ...
        # model input is NCHW [1,3,640,640]. NATIVE path: feed raw uint8 (the graph's input
        # quant = scale 1/255, so uint8 pixels ARE the native tensor) -> a memcpy, no float
        # quantize. FLOAT path (w8a16): values [0,1], appbuilder quantises float->native for us.
        if self.native:
            x = np.ascontiguousarray(lb.transpose(2, 0, 1)).reshape(1, 3, IN, IN)   # uint8
        else:
            x = np.ascontiguousarray(
                (lb.astype(np.float32) / 255.0).transpose(2, 0, 1)).reshape(1, 3, IN, IN)
