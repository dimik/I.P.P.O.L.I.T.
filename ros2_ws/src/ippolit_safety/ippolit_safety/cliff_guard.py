...
MIDAS_STOP = float(os.environ.get('Q6A_CLIFF_MIDAS_STOP', '0.35'))   # center drop for a cliff
MIDAS_FUSE = float(os.environ.get('Q6A_CLIFF_MIDAS_FUSE', '0.28'))   # weaker, needs LiDAR agree
MIDAS_CLEAR = float(os.environ.get('Q6A_CLIFF_MIDAS_CLEAR', '0.25'))  # re-arm hysteresis
# drop must be SHARP (real edge ~5-7, smooth floor gradient ~1.1-2.2)
MIN_SHARP = float(os.environ.get('Q6A_CLIFF_MIN_SHARP', '4.0'))
...
LIDAR_STALE_S = 2.0
SPEAK_GAP = float(os.environ.get('Q6A_CLIFF_SPEAK_GAP', '8.0'))      # min secs between edge warns
# one "Ouch!" per bump episode — the bumper flickers / re-contacts during back-off+turn recovery,
# so a bare rising edge over-announces
OUCH_COOLDOWN = float(os.environ.get('Q6A_OUCH_COOLDOWN', '3.0'))
...
class CliffGuard(Node):
    def __init__(self):
        ...
        self.get_logger().info(
            ...
            f'(>={MIDAS_STOP}, or >={MIDAS_FUSE}+LiDAR-open) -> ADVISORY /cliff/ahead '
    ...
    def on_bumper(self, m):
        ...
        if m.data and not self.bumped:
            self.bumped = True
            now = time.monotonic()
            if now - self.ouch_t >= OUCH_COOLDOWN:   # collapse a flickering bump into one "Ouch!"
                self.ouch_t = now
