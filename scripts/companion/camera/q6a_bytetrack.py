...
Assigns persistent track IDs to per-frame YOLO detections and smooths/predicts boxes with a constant-
velocity Kalman filter. ByteTrack's trick: associate HIGH-confidence detections first, then recover tracks
with the LEFTOVER low-confidence detections a plain tracker would discard — so an object briefly seen at
0.3 conf keeps its ID instead of flickering. Matching is per-class, greedy on IoU (Hungarian-free: near
optimal for the <50 mostly-disjoint boxes we see, and scipy isn't on the device).

Runs in the detector process right after infer() (<1 ms for tens of boxes on a Silver core). Output rows
carry a stable track_id that flows through the detection shm to the overlay + downstream.
"""
import numpy as np


def _iou_matrix(a, b):
    """IoU between every box in a (N,4 xyxy) and b (M,4 xyxy) -> (N,M)."""
...
def _greedy_match(iou, thr):
    ...
    if iou.size:
        ...
        while True:
            ...
            matches.append((i, j))
            iou[i, :] = -1; iou[:, j] = -1
    mr = set(m[0] for m in matches); mc = set(m[1] for m in matches)
...
class _Track:
    ...
    def __init__(self, box, score, cls):
        ...
        self.cls = int(cls); self.score = float(score)
    ...
    _F = np.eye(8, dtype=np.float32); _F[0, 4] = _F[1, 5] = _F[2, 6] = _F[3, 7] = 1.0
    _H = np.zeros((4, 8), np.float32); _H[0, 0] = _H[1, 1] = _H[2, 2] = _H[3, 3] = 1.0
    ...
    def predict(self):
        ...
        self.x[2] = max(self.x[2], 1.0); self.x[3] = max(self.x[3], 1.0)   # keep w,h positive
    ...
    def update(self, box, score, cls):
        ...
        self.score = float(score); self.cls = int(cls)
...
class ByteTracker:
    def __init__(self, high_thresh=0.5, low_thresh=0.1, match_iou=0.2, max_age=30, min_hits=2):
        self.high = high_thresh          # detections >= this drive the first association + spawn tracks
    ...
    def _associate(self, det_idx, track_idx, boxes, scores, classes, spawn):
        """Match the given detection indices to the given track indices (same-class, greedy IoU)."""
        if len(det_idx) == 0:
            return track_idx
        ...
        if avail:
            ...
            db = boxes[det_idx]; dc = classes[det_idx]
