...
#   libserialtap.so        — read-only LiDAR siphon: tees AVA's ttyS3 reads to a tmpfs shm ring
...
PRELOAD=""
for so in /data/lib/libfanoff_filter.so /data/lib/libcamsiphon.so /data/lib/libserialtap.so; do
